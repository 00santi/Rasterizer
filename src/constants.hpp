#pragma once
#include "types.hpp"

inline constexpr int WIDTH = 1000;
inline constexpr int HEIGHT = 800;
inline constexpr Color green {28, 232, 179, 255};
inline constexpr Color black {0, 0, 0, 255};
inline constexpr Color red {150, 0, 0, 255};
inline constexpr Color white {255, 255, 255, 255};
inline constexpr Color purple {148, 62, 143, 255};
inline constexpr Color pink { 242, 143, 173, 255 };
inline constexpr Color yellow { 238, 207, 109, 255 };
inline constexpr Color blue { 122, 162, 247, 255 };
inline constexpr Color lavender { 187, 154, 247, 255 };
